...
func handleResponses(responsesChannel chan response, loops int) {
	...
	for res := range responsesChannel {
		...
		if res.body != "" && !res.ignore {
			...
			appendToFile("log.txt", fmt.Sprintf("\n %s \n", res.body))
...
retry:
	...
	Request, _ := http.NewRequest(http.MethodPost, "https://i.instagram.com/api/v1/accounts/login/", bytes.NewBufferString(data))

	Request.Header.Set("Host", "i.instagram.com")
	...
	transport := &http.Transport{
		DialContext: (&net.Dialer{
			...
			KeepAlive: -1, // 30 * time.Second,
			DualStack: true,
		}).DialContext,

		IdleConnTimeout:       100 * time.Second,
		TLSHandshakeTimeout:   100 * time.Second,
		ExpectContinueTimeout: 0, // 1 * time.Second

		MaxIdleConnsPerHost: 256, // 64 * time.Second
		MaxIdleConns:        256, // 64 * time.Second
		MaxConnsPerHost:     256, // 64 * time.Second
		TLSClientConfig: &tls.Config{
			InsecureSkipVerify: true, // without
		},
		DisableKeepAlives: true, // without
	}
	...
	client := &http.Client{
		Transport: transport,
		Timeout:   time.Millisecond * time.Duration(defaultMillisecondsTimeout),
	}
...
retry:
	...
	Request, _ := http.NewRequest(http.MethodPost, "https://www.instagram.com/accounts/login/ajax/", bytes.NewBufferString(data))

	Request.Header.Set("Host", "www.instagram.com")
	Request.Header.Set("User-Agent", randomdata.UserAgentString())
	Request.Header.Set("X-IG-WWW-Claim", "hmac.AR0gbMXlxalK1rnxdtE9GBbOFaQVz8vCfl7E4EgrVs7T_Uih")
	Request.Header.Set("X-ASBD-ID", "437806")
	Request.Header.Set("X-IG-App-ID", "936619743392459")
	Request.Header.Set("X-Instagram-AJAX", "caee87137ae9")
	Request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	Request.Header.Set("X-CSRFToken", randomdata.RandStringRunes(15))
	Request.Header.Set("X-Instagram-AJAX", "1")
